...
if __name__ == "__main__":
    ...
    parser.add_argument('baud_rate', help='baud rate. probably 9600')
    ...
    do_work = True
    payload_len = 20
    with serial.serial_for_url(config.serial_port, int(config.baud_rate),  bytesize=serial.EIGHTBITS, parity=serial.PARITY_NONE, stopbits=serial.STOPBITS_ONE, timeout=5) as ser:
        buffer = bytes()
        while do_work:
                try:
